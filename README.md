...
```text
【脱敏示例，仅供演示】
使用 gongwen-draft，围绕“某项公共服务能力建设”先检索国务院、主管部委官网和地方政府官网的最新政策文件，形成政策依据台账，标明标题、发布机关、发布日期、来源网址和是否可写入正文。

在完成引用核验后，帮我起草一份请示。
发文机关：[发文机关]
主送机关：[上级机关]
请示事项：[请示事项]
缺失的政策依据、金额、文号和审批结论都用占位符，不要编造。
```

```text
【脱敏示例，仅供演示】
使用 gongwen-draft，审核这份请示有没有文种、格式、标点、事实、政策引用和字体风险。

请先列出必须修改的问题，再给出一版规范化改写稿。
如需导出 Word，请按公文格式排版；缺少字体时停止导出并提示补齐。
...
gongwen-draft 支持 23 种公文与政务材料写作样式，其中包括 15 种法定公文文种和 8 类常见机关材料。这里的“样式”包含法定公文和日常政务材料；调研报告属于扩展政务材料，不属于 15 种法定公文文种。
...
| 文种覆盖 | 支持 23 种公文与政务材料写作样式，包括 15 种法定公文文种和 8 类常见机关材料。 |
