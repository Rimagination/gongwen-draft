...
默认导出 Word 草稿时不会生成红头版头。只有明确指定“红头样式”，并提供发文机关标志、发文字号等授权信息或占位符时，gongwen-draft 才会生成红头样式。签发人不是红头样式的必选项，一般只在上行文等确需标注签发人的场景中填写。

```text
请使用 gongwen-draft，把下面内容整理成一份通知，并导出带红头样式的 Word 草稿。

红头版头信息：
发文机关标志：XX区人民政府办公室文件
发文字号：[由用户单位填写]
签发人：不填写（本示例为下行通知，一般不需要）

正文要求：
标题：关于做好夏季防汛准备工作的通知
主送机关：各街道办事处、区直有关部门
主要事项：排查排水管网、修订防汛预案、落实24小时值班

请不要生成印章，不要编造真实发文字号、签发人或审批状态。
```
